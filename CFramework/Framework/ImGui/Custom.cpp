...
bool ImGui::CustomButton(const char* icon, const char* label, const ImVec2& size_arg, const bool& flag)
{
    ...
    /*
    icon の label_size.x が均一とは限らない（し、めんどくさい）のでマジックナンバーを入れるしかない。
    要修正
    */

    // Icon
    RenderTextClipped(ImVec2(bb.Min.x + style.FramePadding.x + 10, bb.Min.y), ImVec2(bb.Min.x - style.FramePadding.x - 10, bb.Max.y), icon, NULL, &label_size, style.ButtonTextAlign, &bb);
    // Text
    RenderTextClipped(ImVec2(bb.Min.x - 20, bb.Min.y), ImVec2(bb.Max.x - 20, bb.Max.y), label, NULL, &label_size, style.ButtonTextAlign, &bb);
...
bool ImGui::CustomButton(const char* label, const ImVec2& size_arg, const bool& flag)
{
    ...
    RenderTextClipped(ImVec2(bb.Min.x + style.FramePadding.x + 10, bb.Min.y + style.FramePadding.y), ImVec2(bb.Min.x - style.FramePadding.x - 10, bb.Max.y - style.FramePadding.y), label, NULL, &label_size, style.ButtonTextAlign, &bb);

    // Text
    //RenderTextClipped(ImVec2(bb.Min.x + style.FramePadding.x, bb.Min.y + style.FramePadding.y), ImVec2(bb.Min.x - style.FramePadding.x - 10, bb.Max.y - style.FramePadding.y), label, NULL, &label_size, style.ButtonTextAlign, &bb);
